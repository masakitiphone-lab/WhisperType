...
pub fn set_overlay_mouse_passthrough(window: &WebviewWindow, enabled: bool) {
    #[cfg(target_os = "windows")]
    set_windows_mouse_passthrough(window, enabled);
...
#[cfg(target_os = "windows")]
fn set_windows_mouse_passthrough(window: &WebviewWindow, enabled: bool) {
    let Ok(hwnd) = window.hwnd() else {
        return;
    };
