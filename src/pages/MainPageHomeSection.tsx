  ...
  return (
    <section ref={setSectionRef} id="home" className="scroll-mt-8">
      <div className="space-y-6">
        ...
        <div className="grid gap-5 overflow-hidden rounded-[30px] border border-white/35 bg-[linear-gradient(135deg,rgba(248,250,252,0.86),rgba(244,247,252,0.92),rgba(248,250,252,0.86))] px-5 py-5 shadow-[0_18px_40px_rgba(15,23,42,0.06)] transition-all duration-200 ease-out dark:border-white/10 dark:bg-[linear-gradient(135deg,rgba(255,255,255,0.06),rgba(255,255,255,0.03),rgba(255,255,255,0.05))] lg:min-h-[430px] lg:grid-cols-[0.92fr_1.08fr] lg:items-stretch">
          <div className="flex flex-col justify-between gap-5">
            <div className="space-y-4">
              <div className="space-y-2">
                <p className="max-w-[14ch] text-[clamp(2rem,4.4vw,3.6rem)] font-semibold leading-[0.96] tracking-tight text-slate-950 dark:text-slate-50">
                  {appLocale === "ja" ? "音声入力が準備できています" : "Voice input is ready"}
                </p>
                <p className="max-w-xl text-sm leading-6 text-slate-500 dark:text-slate-400">
                  {appLocale === "ja" ? `${shortcutLabel} を押して、そのまま話してください。` : `Press ${shortcutLabel} and start speaking.`}
                </p>
              </div>
                ...
                <div className="mt-1 flex min-w-0 items-center justify-between gap-3 overflow-hidden">
                  ...
                  {currentPlan === "plus" ? (
                    <p className="min-w-0 truncate text-right text-xs text-slate-500 dark:text-slate-400">
                      {appLocale === "ja" ? "文字起こしは無制限です" : "Unlimited transcription"}
                    </p>
