...
begin
  if exists (
    ...
    alter table public.profiles rename column credits to bonus_credits;
  end if;
end $$;
...
begin
  ...
  if coalesce(profile_row.daily_credits, 0) + coalesce(profile_row.bonus_credits, 0) < amount then
    raise exception 'insufficient credits';
  end if;

  remaining_amount := amount;
  next_daily_credits := coalesce(profile_row.daily_credits, 0);
  next_bonus_credits := coalesce(profile_row.bonus_credits, 0);
  ...
  set daily_credits = next_daily_credits,
      bonus_credits = next_bonus_credits,
      updated_at = timezone('utc', now())
  where id = user_id_param;
